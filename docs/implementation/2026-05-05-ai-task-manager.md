...
Ready drafts can be opened in a review panel. The panel loads draft detail, shows the original prompt and structured card fields, supports edits through `PATCH /api/ai-card-drafts/:id`, and creates a responsibility through `Put in play`. A ready draft can also be discarded from the tracker or review panel before it is accepted. Failed drafts stay scoped to their own request card and do not block new prompts.
...
- `DELETE /api/ai-card-drafts/:id`: discard failed, canceled, or ready drafts before acceptance.
...
Submitting a prompt clears the prompt bar immediately and adds a compact optimistic request card. Each request tracks its own status, generated result, retry action, cancel action, discard action, and error details. A failed request remains reviewable and retryable without blocking future prompts.
...
- Accepted drafts cannot be discarded from the AI tracker because they have already become responsibilities.
...
9. Discard a ready draft and confirm it leaves the tracker without creating a responsibility.
10. Put a ready draft in play and confirm navigation to the new responsibility.
